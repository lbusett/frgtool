#'@title FRG_MOD_Comp_DVI
#'@description Accessory function used to compute DVI from RED and NIR images exploiting IDL external function
#'@details This function is used simply to call the appropriate IDL function used to compute DVI values from MODIS RED and NIR bands data
#' values. See documentation of "FRG_COMPUTE_DVI.pro" IDL script for additional details
#'
...
FRG_MOD_Comp_DVI <- function(OutOrig_Path= OutOrig_Path, ReProc = ReProc, mess_lab = mess_lab, yy = yy) {
	...
	DVI_Dir = file.path(OutOrig_Path,'DVI','Single_Dates')				# Folder where to put DVI data
	dir.create (DVI_Dir, showWarnings = FALSE) 							# Create output folder if necessary
	...
	if (length(RED_files_names) > 0) {
		...
		out_files_names = gsub('Red','DVI',RED_files_names, fixed = TRUE)			# List of output DVI files
		
		for (file in 1:length(RED_files_names)) {
			
			RED_file = file.path(RED_Dir, RED_files_names[file])			;   NIR_file = file.path(NIR_Dir, NIR_files_names[file])	; out_file = file.path(DVI_Dir, out_files_names[file])
			
			if (file.exists(out_file) == F | ReProc == 1) {				# Check for output file existance or REproc FLAG
				
				# Create IDL processing string
				str_idl = paste(	'res = FRG_COMPUTE_DVI(',	'NIR_file = \'',NIR_file, 
						'\' , ',	'RED_File =\'',  RED_file,
						'\' ,',	'Out_File=\'',  out_file,
				...
				batch_file = file.path(FRG_Options$IDL_Dir,'FRG_CreateDVI_batch.pro')
				fileConn<-file(batch_file)
				writeLines(c(exp_path_str,'envi, /restore_base_save_files  ', "ENVI_batch_init",str_idl,'exit'), fileConn)
				close(fileConn)
				svalue(mess_lab) = paste('--- Computing DVI for date ',  out_files_names[file], ' ---')    # Update progress window
				out = system ((paste('idl ',batch_file,sep = '')),  invisible = TRUE, show.output.on.console = TRUE)		# Call IDL processing 
        
        if (out != 0) {
         print('An error occurred while Computing DVI ! Processing stopped')
        stop()
      }
