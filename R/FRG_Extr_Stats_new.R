...
#' @export
#'
FRG_Extr_Stats_new = function(SVI_File = SVI_File, Shape_File = Shape_File, CLC_File_00 = CLC_File_00,
                          ENV_Zones_File = ENV_Zones_File, Out_File = Out_File, erode = erode, erode_file = erode_file,
                          Intermediate_Folder = Intermediate_Folder, Overlap = Overlap, Shape_File_Orig = '', 
                          LUT_File_Multiple = '') {
...
browser()
  ...
  #----------------------------------------------------------------------------------------------------------#
    ...
    if (Overlap == 'Single') {
      ...
      Data$ENV_ZONE  = factor(Data$ENV_ZONE, levels = c(1:13), 
                              labels = c('ALN', 'BOR','NEM', 'ATN', 'ALS',
                            							'CON', 'ATC','PAN', 'LUS', 'ANA',
                            							'MDM', 'MDN', 'MDS'))
        
        # recode(Data$ENV_ZONE ,  # Recode ENV_ZONE according to legend
