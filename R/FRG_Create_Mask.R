# - Accessory function for Mask creation. Calls the FRG_CREATE_MASK.pro IDL function

FRG_Create_Mask <- function(ROI_File = ROI_File, CLC_File_00 = CLC_File_00, exp_path_str = exp_path_str, Intermediate_Folder = Intermediate_Folder) {
  
  dir.create(file.path(Intermediate_Folder,'ENVI_Mask'), showWarnings = FALSE, recursive = TRUE)
  FireMask_File = file.path(Intermediate_Folder,'ENVI_Mask', paste(sub("[.][^.]*$", "", basename(ROI_File)),'_ENVI_Mask', sep = ''))
  selection = 'no'
  ...
  if (!file.exists(FireMask_File) | selection == 'yes') {
    ...
    print(paste('--- Creating Burnt Areas Mask File: ',FireMask_File,' ---'))    							# Update status bar
