...
FRG_Full_Proc_GUI = function () {
  ...
  Files_Group = gframe(text = "Select the Processing Files - Folders", horizontal = FALSE, container=Main_W)
  ...
  ReProc_Group = gframe(text = "Select the ReProcessing Options", horizontal = FALSE, container=Main_W)
  Fake_Group3 = glabel(text ='', container=Main_W)
  
  # -------------------------------------------------------------------------------------------------------------------- #
  # Widgets for Full_Selection of Input ShapeFile (Used to mask out burned areas when computing statistics !) 
  # -------------------------------------------------------------------------------------------------------------------- #
  ...
  Shape_File <- gedit(text = format(Full_Selection$Shape_File, justify = "right"), container=Shape_SelGroup, anchor=c(1,1))
  ...
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of the folder where to place original and pre-processed MODIS satellite data 
  # ------------------------------------------------------------------- #
  
  MOD_SelGroup = ggroup(horizontal = TRUE, container=Files_Group)					# Main group
  Out_Lab <- glabel(text ='Main Folder for download of MODIS data', container=MOD_SelGroup, editable =FALSE)  # Label
  size(Out_Lab) <- c(280,8)																		# Set label width
  MOD_Dir <- gedit(text = format(Full_Selection$MOD_Dir, justify = "right") , container=MOD_SelGroup)			# Selected file 
  ...
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of CORINE 2000 image
  # ------------------------------------------------------------------- #
  ...
  CLC_File_00 <- gedit(text = format(Full_Selection$CLC_File_00, justify = "right"), container=CLC_SelGroup_00, anchor=c(1,1))
  ...
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of Output Folder 
  # ------------------------------------------------------------------- #
  Out_SelGroup = ggroup(horizontal = TRUE, container=Files_Group)
  Out_Lab <- glabel(text ='Output Folder for time series statistical analysis', container=Out_SelGroup,width = 400)
  size(Out_Lab) <- c(280,8)
  Out_Folder <- gedit(text = format(Full_Selection$Out_Folder, justify = "right"), container=Out_SelGroup, anchor=c(1,1))
  size(Out_Folder) <- c(600,20)
  Out_Choose <- gbutton("Browse",handler = function(h,...) {choice<-gfile(type="selectdir", text="Select the Output Folder for the results of the statistical analysis...")
  ...
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of Processing Years
  # ------------------------------------------------------------------- #
  ...
  Start_Year <- gspinbutton(from=2000,to=2000,by=1,value=Full_Selection$Start_Year, container=Year_SelGroup, anchor=c(1,1))
  ...
  End_Year <- gspinbutton(from=2012,to=2020,by=1,value=Full_Selection$End_Year, container=Year_SelGroup, anchor=c(1,1))
  ...
  NKer = gspinbutton(from = 10, to = 200, by = 1,value =  Full_Selection$NKer, horizontal = TRUE, container = Year_SelGroup)
  
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of Percentages
  # ------------------------------------------------------------------- #
  
  perc_Lab <- glabel(text ='     Minimum NDVI Difference       ', container=Year_SelGroup,font.attr=list(style="bold",size = 'big'), editable =FALSE)
  perc_diff = gspinbutton(from = 0, to = 30, by = 0.1,value =  9.5, horizontal = TRUE, container = Year_SelGroup)
  
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of Processing Method
  # ------------------------------------------------------------------- #
#   Meth_Lab <- glabel(text ='     Processing Method ', container=Year_SelGroup,font.attr=list(style="bold",size = 'big'), editable =FALSE)
#   Meth_opt = c('0-100 Rescaling', 'Percentage Deviation to Kernel Median ', ' Both Methods')
#   
#   Method= gdroplist(Meth_opt, selected = Full_Selection$Method, container = Year_SelGroup)
  
  
  
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of Indexes to be analyzed
  # ------------------------------------------------------------------- #	
...
#   SNDVI <- gradio(items = c('Yes','No'),container = Ind_SelGroup, horizontal = T, anchor=c(1,1), selected =Full_Selection$SNDVI)
#   SRDVI_lab <- glabel(text ='           Compute Scaled RDVI (SRDVI)', container=Ind_SelGroup,font.attr=list(style="bold",size = 'big'), editable =FALSE)
#   SRDVI <- gradio(items = c('Yes','No'),container = Ind_SelGroup, horizontal = T, anchor=c(1,1), selected = Full_Selection$SRDVI)
#   
  
  # ------------------------------------------------------------------- #
  # Widgets for Full_Selection of ReDownload and ReProcess options
  # ------------------------------------------------------------------- #	
  ...
  ReProc <- gradio(items = c('Yes','No'),container = Re_SelGroup, horizontal = T, anchor=c(1,1), selected = 2)
  ReProcIm_lab <- glabel(text ='           Reprocess Existing Scaled Indexes', container=Re_SelGroup,font.attr=list(style="bold",size = 'big'), editable =FALSE)
  ...
  Start_But <- gbutton("Start", handler=function(button,...){ 
    ...
    Full_Selection$MOD_Dir = svalue(MOD_Dir)											# If Start selected, retrieve widgets values
    Full_Selection$Shape_File = svalue(Shape_File)											# If Start selected, retrieve widgets values
    Full_Selection$CLC_File_00 = svalue(CLC_File_00)											# If Start selected, retrieve widgets values
    Full_Selection$Out_Folder = svalue(Out_Folder)											# If Start selected, retrieve widgets values
...
#     Full_Selection$Method= svalue(Method, index = TRUE)
    
    Full_Selection$Method= 2                                    # Always set to 2, which means analysis based on percentage difference to kernel median. (In oldere versions
                                                                # A second method was considered, but it was stripped
#     Full_Selection$SRDVI = svalue(SRDVI, index = TRUE)
#     Full_Selection$SNDVI = svalue(SNDVI, index = TRUE)
    ...
    if (Full_Selection$End_Year >= Full_Selection$Start_Year) {
      save (Full_Selection, file = file.path(FRG_Options$Previous_Dir,'FRG_Full_Previous.RData'))		# Save  widgets values in the FRG_CS_Previous.RData file
      ...
      with(Full_Selection, FRG_Full_Processing(MOD_Dir = MOD_Dir,Shape_File = Shape_File,CLC_File_00 = CLC_File_00, 
                                               ...
                                               ReProc = ReProc, ReDown = ReDown, ReProcIm = ReProcIm))
