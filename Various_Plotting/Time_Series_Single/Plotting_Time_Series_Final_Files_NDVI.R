...
#- ----------------------------------------- - 
  ...
  plot_folder = 'D:/Documents/GDrive/FRG/Results_2000_2015_Ker_200_Perc_Diff_9.5/Plots/Time_Series/'   # out folder for plots
  dir.create(plot_folder, recursive = T)
  
  max_N_Years_after = 11
  ...
  SNDVI_Folder = "D:/Documents/GDrive/FRG/Results_2000_2015_Ker_200_Perc_Diff_9.5/Med_SNDVI/Stat_Analysis"
  files_SNDVI = c('D:/Documents/GDrive/FRG/Results_2000_2015_Ker_200_Perc_Diff_9.5/Med_SNDVI/Stat_Analysis/Burned_Once/Stat_Analysis_Med_SNDVI_2000_2015_META_RData.RData')
  In_File_SNDVI = file.path(files_SNDVI) 

  
  # File needed to get the shapefile information
  In_File_x_Shape = 'D:/Documents/GDrive/FRG/Results_2000_2015_Ker_200_Perc_Diff_9.5/Med_SNDVI/TS_Extraction/Burned_Once/TS_Extraction_Med_SNDVI_2000_2015_META_RData.RData'  
  ...
  leg_size =0.50
  leg_fonts = 6.54
  for (Fire in c(1)) a = plot_single(Fire,In_File_SNDVI,In_File_SNDVI,medwdt,wdt,ht,leg_size, leg_fonts)
...
#     pdf(out_file,wdt,ht, pointsize = 4)
   ...
   browser()
    ...
    Data_Fire_Rec = join(as.data.frame(Data_Fire_Rec),sig_arrays, type = 'left')
    ...
    Rec_Years = ddply(as.data.frame(Data_Fire_Rec), .(CLC_Class), summarize, Rec_Year = min(which(DiffSignif ==2)),
    ...
    p1 = ggplot(as.data.frame(Data_Fire_Rec))+ my_theme_bw 
    ...
    p1 = p1 + geom_boxplot(aes_string(x =x_var, ymin = 'low_box', lower = 'X25th',
                                      middle = 'median', upper = 'X75th', ymax = 'up_box',fill = 'DiffSignif'),stat = 'identity', position='dodge', size = 0.3)+
      facet_wrap(~CLC_Class, ncol = 2)
