...
#' 
frg_wilcox = function(In_File   = In_File,   out_file_pmat = out_file_pmat, min_pix = min_pix, 
                      perc_diff = perc_diff, MedWdt = MedWdt, sub_zones = sub_zones, erode = erode) 
{
  ...
  get(load(In_File))
  Index    = attributes(ts_data)$Index				
  SVI_File = attributes(ts_data)$SVI_File   
  Shape_File = attributes(ts_data)$Shape_File
  CSV_File = attributes(ts_data)$CSV_File
  CLC_File = attributes(ts_data)$CLC_File
  ...
  attributes(ts_data_Melted)$Index = Index
  ...
  p_matrix_median_empty =  array(ts_data = NA, dim = c(max_plus_years-max_minus_years, max_plus_years-max_minus_years))		#Define an empty matrix with correct dimensions to store the results of 
  ...
  ts_data_Melted$YearDiff = (as.numeric(as.character(ts_data_Melted$Year)) - as.numeric(ts_data_Melted$FireYear))		# Compute the "Years from fire" variable.
